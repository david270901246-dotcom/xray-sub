...
功能：
  ...
  - 本地 HTTP 服务，手机通过局域网访问订阅地址
  - 自动测速，按速度排序

用法：
  python xray_sub_server.py           # 正常启动（打开浏览器 http://127.0.0.1:8899）
  python xray_sub_server.py --port 8899   # 指定端口
  python xray_sub_server.py --bind 0.0.0.0 # 绑定所有网卡（手机可访问）
  python xray_sub_server.py --test        # 仅测速，不启动服务器

手机设置：
  1. 确认电脑 IP（如 192.168.1.100）
  2. 启动时加 --bind 0.0.0.0
  3. v2rayNG → 订阅 → 添加订阅：
     http://192.168.1.100:8899/sub/v2rayng
     http://192.168.1.100:8899/sub/clash
"""
...
PORT = 8899
BIND = "127.0.0.1"   # 改为 "0.0.0.0" 手机才能访问
...
# 测试参数
TCP_TIMEOUT = 3
MAX_TEST_NODES = 30    # 最多测试多少个节点
MAX_SUBSCRIBE_NODES = 50  # 订阅里最多放多少个节点（按速度排序）
...
def clean_sni(sni):
    if not sni:
        return sni
    try:
        decoded = urllib.parse.unquote(snn := sni)
        if "%" in decoded:
...
def generate_v2rayng_content(nodes):
    """生成 v2rayNG 订阅内容（Base64 编码的 JSON 配置）"""
...
class SubHandler(http.server.BaseHTTPRequestHandler):
    ...
    def send_subscription(self):
        ...
        self.send_header("Subscription-Userinfo", f"upload=0; download=0; total=107374182400000; expire=9999999999")
...
<style>
  ...
  .status{{padding:8px 0;color:#666;font-size:13px}}
</style>
</head>
<body>
<h1>📡 Xray 手机订阅服务</h1>
...
<div class="card">
  <div class="label">v2rayNG 订阅地址（推荐）</div>
  <div class="url">http://{local_ip}:{PORT}/sub/v2rayng</div>
...
<div class="card">
  ...
  <div class="url">http://{local_ip}:{PORT}/sub/clash</div>
  <div style="margin-top:12px">
...
<div class="card">
  <div class="label">订阅接口文档</div>
  <div style="font-size:13px;color:#666;margin-top:8px">
    <b>手动复制地址：</b>在 v2rayNG/Clash 里手动添加上面的 URL<br>
    <b>手机设置：</b>确保手机和电脑在同一 WiFi 网络下<br>
    <b>自动更新：</b>v2rayNG 可设置定时自动更新订阅（设置→订阅设置→自动更新）<br>
    <b>节点数量：</b>自动抓取 6 个 GitHub 源，保留速度最快的 {MAX_SUBSCRIBE_NODES} 个节点
  </div>
</div>
...
<div class="card">
  ...
  <div style="font-size:13px;color:#555;margin-top:8px;line-height:1.8">
    1. 手机安装 <b>v2rayNG</b>（酷安/应用商店搜索）<br>
...
<div class="card">
  ...
  <div class="status">
    监听端口：<b>{PORT}</b><br>
    绑定地址：<b>{BIND}</b>（手机访问需为 0.0.0.0）<br>
    缓存时间：<b>{CACHE_TTL}秒</b><br>
    最大测速：<b>{MAX_TEST_NODES}个节点</b><br>
    订阅节点：<b>最多{MAX_SUBSCRIBE_NODES}个（按速度排序）</b>
  </div>
</div>

<div class="tip" style="text-align:center;margin-top:24px">
  本地订阅服务 · 每次访问自动刷新节点（5分钟缓存）<br>
  数据来源：GitHub 免费节点源（NoMoreWalls / freenode / freefq 等）
</div>
</body>
</html>"""
...
def main():
    ...
    for arg in sys.argv[1:]:
        ...
        elif arg == "--bind" and len(sys.argv) > 1:
    ...
    print(f"\n  🔧 如果手机无法访问，")
    print(f"     重启服务时加参数：python xray_sub_server.py --bind 0.0.0.0")
    print(f"\n  按 Ctrl+C 停止服务")
    print("=" * 60)
