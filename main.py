...
class SubHandler(http.server.BaseHTTPRequestHandler):
    ...
    def send_subscription(self):
            ...
            if _nodes_cache is None:
                ...
                def async_warmup():
                    try:
                        ...
                        print(f"[后台] 节点预热完成，{len(sorted_nodes)} 个")
                ...
                try:
                    ...
                    _nodes_cache = sorted_nodes
                    _cache_time = now
                    selected = sorted_nodes[:MAX_SUBSCRIBE_NODES]
                except Exception as e:
                    print(f"[警告] 刷新失败：{e}，使用旧缓存")
                    selected = _nodes_cache[:MAX_SUBSCRIBE_NODES] if _nodes_cache else []
        else:
        ...
        if not selected:
            empty_config = {
                "log": {"loglevel": "warning"},
                "inbounds": [{"tag": "socks", "port": 1080, "listen": "127.0.0.1", "protocol": "socks", "settings": {"auth": "noauth"}}],
                "outbounds": [{"tag": "direct", "protocol": "freedom"}]
            }
            content = base64.b64encode(json.dumps(empty_config).encode("utf-8")).decode("ascii")
        else:
    ...
    def send_html(self):
        public_host = (os.environ.get("RAILWAY_PUBLIC_DOMAIN")
                       or os.environ.get("RENDER_EXTERNAL_URL")
                       or os.environ.get("PUBLIC_HOST"))
        if public_host:
            if public_host.startswith("http"):
                base_url = public_host.rstrip("/")
            else:
...
<div class="card">
 ...
 部署平台：<b>{'Railway' if os.environ.get('RAILWAY_PUBLIC_DOMAIN') else 'Render' if os.environ.get('RENDER_EXTERNAL_URL') else '本地'}</b><br>
 </div>
</div>
</body>
</html>"""
...
def main():
    ...
    if public_host:
        if public_host.startswith("http"):
            print(f"\n 🌐 公网地址：")
            print(f" {public_host.rstrip('/')}/")
        else:
            print(f"\n 🌐 公网地址：")
            print(f" https://{public_host}/")
    else:
        print(f"\n 📱 手机订阅地址（同 WiFi 下复制）：")
    ...
    def warmup_thread():
        try:
            ...
            _nodes_cache = sorted_nodes
            _cache_time = time.time()
            print(f"[后台] 预热完成，{len(sorted_nodes)} 个节点")
        except Exception as e:
            print(f"[后台] 预热失败：{e}")
